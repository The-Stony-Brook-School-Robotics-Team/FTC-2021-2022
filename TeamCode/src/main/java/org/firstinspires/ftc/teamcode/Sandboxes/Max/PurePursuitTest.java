...
//    private Motor.Encoder CentralEncoder;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Waypoint intermediateW = new GeneralWaypoint(LeftEncoder.getCurrentPosition()+8192 * 5, RightEncoder.getCurrentPosition()+ 8192* 5, Math.PI/2, 1, 50, Math.PI);
        Waypoint postW = new InterruptWaypoint();
        Waypoint endW = new EndWaypoint(LeftEncoder.getCurrentPosition()+8192 * 11, RightEncoder.getCurrentPosition()+ 8192* 11, Math.PI/2, 1, 50, 2 * Math.PI, 2 * Math.PI * 2, 2 * Math.PI * 2);
