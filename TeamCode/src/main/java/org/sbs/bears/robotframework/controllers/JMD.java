...
@Config
public class JMD extends MecanumDrive {
    public static PIDCoefficients TRANSLATIONAL_PID = new PIDCoefficients(0, 0, 0);
    public static PIDCoefficients HEADING_PID = new PIDCoefficients(0, 0, 0);

    public static boolean isRunningFollowTrajectory = false;
    ...
    private double strifeWeight = 0.5;
    private double headWeight = 1;
    private boolean isHeadless = true;

    private BNO055IMU imu;
    private VoltageSensor batteryVoltageSensor;

    public JMD(HardwareMap hardwareMap, Gamepad gamepad) {
        super(kV, kA, kStatic, TRACK_WIDTH, TRACK_WIDTH, LATERAL_MULTIPLIER);

        follower = new HolonomicPIDVAFollower(TRANSLATIONAL_PID, TRANSLATIONAL_PID, HEADING_PID,
                new Pose2d(0.1, 0.1, Math.toRadians(1.0)), 0.0);
    ...
    public void update() {
        updatePoseEstimate();
        DriveSignal tempSignal = trajectorySequenceRunner.update(getPoseEstimate(), getPoseVelocity());
        if (tempSignal == null)
            return;
        ...
        double yDiffVal;
        if (isHeadless) {
            xDiffVal = gamepad.right_stick_y * strifeWeight;
            yDiffVal = -gamepad.right_stick_x * strifeWeight;
        } else {
            xDiffVal = gamepad.right_stick_y * strifeWeight * Math.sin(currentHeading);
            yDiffVal = -gamepad.right_stick_x * strifeWeight * Math.cos(currentHeading);
        }

        DriveSignal signal = new DriveSignal(
                new Pose2d(tempSignal.getVel().getX() + xDiffVal,
                        tempSignal.getVel().getY() + yDiffVal,
                        tempSignal.getVel().getHeading() - gamepad.left_stick_x * headWeight),
                tempSignal.getAccel()
        );
